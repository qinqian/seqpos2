            ...
            if ( bp2int >= 0 && bp2int < 4 ){
                ...
                if ( j >= (motiflength - 1)){      
                    if ( score_option == MEAN_OPTION ){
                        ...
                        if ((motif_record[k]).score < exp(combinedscore) + eps){
                        (motif_record[k]).score  = exp(combinedscore);
                        (motif_record[k]).istart = j - motiflength + 1; 
                        (motif_record[k]).iend   = j+1; 
                        (motif_record[k]).orient = FWD;
                        (motif_record[k]).iseq = (int)iseq;
                        (motif_record[k]).lenseq = lenseq;
                        }
                        ...
                        if ( exp(combinedscore) > CUTOFF  && idx < nlocations){
            ...
            if ( bp2int >= 0 && bp2int < 4 ){
                ...
                if( j <= (lenseq - motiflength)){ 
                    if ( score_option == MEAN_OPTION ){
                        ...
                        if ((motif_record[k]).score < exp(combinedscore) + eps ){
                        (motif_record[k]).score  = exp( combinedscore );
                        (motif_record[k]).istart = j; 
                        (motif_record[k]).iend   = j + motiflength; 
                        (motif_record[k]).orient = RVS;
                        (motif_record[k]).iseq   = (int)iseq;
                        (motif_record[k]).lenseq = lenseq;
                        }
                    }else if( score_option == CUTOFF_OPTION ){
